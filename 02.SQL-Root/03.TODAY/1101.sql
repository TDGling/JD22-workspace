...
-- 권한값은 A:Admin, U:User
-- 기본값 U가 들어가게 함
-- 변경방법 : ALTER TABLE 테이블명 ALTER COLUMN 컬럼명 SET DEFAULT 값
ALTER TABLE `todolist`
...
-- 기본값이 있는 auth컬럼의 값을 기본값 "U" 가 아닌 "A"
INSERT INTO `todolist` (`title`,`cont`,`auth`)
VALUES ("SQL 데이터빼기","seleact로 데이터를 빼보자!","A");
-- 문자데이터항목에 숫자로 넣어도 문자열 숫자로 입력됨
INSERT INTO `todolist` (`title`,`cont`,`auth`)
VALUES (333,"SQL 데이터빼기","seleact로 데이터를 빼보자!","A");

-- auto항목의 한글자 데이터인데 두글자 넣은 경우
INSERT INTO `todolist` (`title`,`cont`,`auth`)
VALUES ("SQL 데이터빼기","seleact로 데이터를 빼보자!","AA");
-- 입력성공 하단에 경고메시지 나옴
-- Warning: #1265 Data Truncated for Column `auth` at row1
-- 해석 : 입력한 데이터가 넘쳐서 잘림
/*
