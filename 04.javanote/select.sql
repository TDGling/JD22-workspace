...
-- 참고) 선택시 컬럼명에 AS를 사용하고 뒤에 문자를 적으면 선택결과 출력시 해당 이름으로 결과가 출력된다!
...
-- > 결과보고 특이사항 : "Credit Card"가 아닌 데이터가 나왔으나 null값도 제외됨(구체적인 값이 없으므로 자동제외)
...
-- 쿼리 14 `orders` 테이블
SELECT COUNT(*) AS "전체 레코드수" FROM `orders` WHERE `payment_type` = "Check" AND NOT `ship_city` = "new york" OR `ship_name` = "Oristina Lee";
