...
public class java04 {
    public static void main(String[] args) {
            ...
            // 스캐너 생성시 값으로 System.in 을 설정하여 입력라인이 생성되게 한다!
            
            // 스캐너는 메모리 영역을 try 문으로 감싸서 관리한다.
            ...
            // 입력창으로 받은 데이터는 엔터 기호등 다른 데이터가 포함되어 있어서 == 비교연산자로 하지말고
            // 변수 .equals(내용) 메서드로 비교하면 정확히 입력된 데이터만 비교한다.

            // 배우 이름에 해당하는 대표작으로 표시하는 if문
            // if(actor=="데이나"){ // 데이터 때문에 비교 못함
            if(actor.equals("데이나")){
                System.out.println("대표작 : 안녕 데이나");
            }
            else if(actor.equals("지누")){
                System.out.println("대표작 : 사장");
            }
            else{
