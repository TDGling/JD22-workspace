...
public class java07 {
    ...
    /*
        ...
        2. private(프라이빗 > 어떤 느낌)-> 다른 클래스에서 접근불가
        -> 클래스 안에서만 접근됨
        -> 프라이빗 속성일 경우 Getter/Setter 를 통해 접근한다.
        이런 방법을 캡슐화라고 한다.
        3. protected(프로텍티드 > 어떤 느낌) -> 다른 클래스에서 접근불가
        -> 하위클래스 (상속받은 클래스) 에서 접근됨.
        -> 같은 패키지 안에서도 접근가능
        4. default -> 아무것도 안쓴 경우
        -> 같은 패키지(같은폴더)안에서만 접근가능함

        [ 캡슐화란? ]
        > 클래스의 속성을 privte으로 만들고 메서드를 통하여 값을 넣거나 가져오는 방법
        > 겟터와 셋터를 생성한다.

        { 캡슐화를 하는 이유 }
        1. 클래서 속성명 메서드의 제어 향상
        2. 클래스 속성은 읽기전용(Getter) 와 쓰기전용 (Setter) f로 만들 수 있고
        이를 통해 내부적인 데이터 흐름의 효율성을 높일 수 있다.
        3. 다른 코드에 영향을 주지않고 한 부분만 변경하는 유연성을 가질 수 있다
        4. 데이터에 대한 보안이 강화된다.
        5. 자바 웹을 구현할 때 캡슐화는 선택이 아닌 필수
        -> 겟터와 셋터가 형식이 일정한데 속성(변수)이 많아지면 이것을 만들기 귀찮음
        자동생성을 툴에서 지원한다.
        ...
        [ 자바의 패키지 ]
        1. 패키지는 자바의 클래스를 그룹하하는 방법이다
        2. 파일의 다렉토와 같이 폴더를 생성하여 묶어준다
        3. 패키지의 장점
            1) 클래스의 내용별 분류 및 체계화가 된다
            2) 동일한 이름의 클래스를 분류별로 보유할 수 있다.
        4. 패키지의 종류
            1) 내장 패키지 (Java API) : 자바가 가지고 있는 기능별 클래스 묶음
            -> https://docs.oracle.com/javase/8/docs/api/
            2) 사용자 정의 패키지 : 자신만의 패키지를 생성한다
            -> src 하위에 폴더를 만들고 거기에 java 파일 생성
        ...
        6. 패키지에 생성한 java 파일 상단에는 자신이 속한 패키지가 표시된다.
        표시형식 : package 패키지명. 폴더명
    ...
    public static void main(String[] args) {
         ...
         * -> 반대로 보면 객체는 클래스의 실물이다(인스턴스다!)
            -> 컴파일시 하나의 java파일안에 여러개의 class를 만들어도 별도의 class를 모두 따로 생성한다.(bin 폴더 확인)
            -> 클래스는 기본 모듈의 단위다
        ...
        // 어떻게 셋팅하고 값을 불러오지? (겟터/셋터)

        // 가격 설정하기 : setPrice() 가격 가져오기 : getPrice()
        ...
        // 패키지를 클래스까지 정확히 import 해 준다.(최상단에)
...
class Fruits {
     ...
     * -> 프라이빗 플드를 캡슐화하여 접근하는 방법
     * -> 장점은 속성을 직접 노출하지 않으며 값을 넣고 읽는 메서드를 생성하여 속성에 대한 일관된 접근방법을 제시해준다.
     * 
     *  {★ 겟터(Getter)}
     *  1. 필드값을 반환한다.
     *  2. 접근 제어 수식어 : public
     *  3. 겟터명 : get필드명 ( 필드명의 첫글자는 대문자)
     *  4. 메개변수 없음
     *  5. 내용 : return 필드
     * 
     *  {★ 셋터(Setter)}
     ...
     *  3. 셋터명 : set필드명 ( 필드명의 첫글자는 대문자 )
     *  4. 메개변수 : 필드와 동일
     *  5. 내용 : 생성자와 동일 ( this.필드명 = 매개변수명 )
     * 
     */
    // Getter
    public int getPrice(){
